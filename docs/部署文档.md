...
本项目采用Docker容器化部署，使用Docker Compose进行服务编排，具有以下优势：
...
- **传统部署**: 需要手动安装JDK、MySQL、Nginx等（不推荐）
- **Kubernetes部署**: 适合大规模生产环境（未来支持）
...
主要配置文件：`health-cert-backend/src/main/resources/application.yml`
...
服务启动有依赖关系，Docker Compose会自动处理：

1. **MySQL** → 首先启动，等待健康检查通过
2. **Backend** → 等待MySQL健康检查通过后启动
3. **Nginx** → 等待Backend健康检查通过后启动
